...
`--onnx-readback sync|pbo` 控制 GPU staging 之后的小图 readback 方式。`sync` 是默认稳定路径；`pbo` 使用 pixel buffer object 做实验性异步 readback，会让 ONNX 输入使用上一帧读回的数据，换取更低的同步等待成本。PBO 只适合 runtime 性能实验，视觉和指标对照仍应先用 `sync`。
